...
class WaterRepository extends ChangeNotifier implements IWaterRepository {
  ...
  Future<void> _saveToFirestore() async {
    ...
    try {
      await _firestore.collection("users").doc(_uid).update({
        ...
        "todayEntries": todayEntries.map((e) => WaterEntryModel.fromEntity(e).toJson()).toList(),
      });
    } catch (e) {
  ...
  Future<void> _saveToPrefs() async {
    try {
      ...
      await prefs.setString(
        "today_entries",
        jsonEncode(todayEntries.map((e) => WaterEntryModel.fromEntity(e).toJson()).toList()),
      );
    } catch (e) {
  ...
  Future<void> loadFromFirestore() async {
    ...
    try {
      ...
      ? entriesData.map((e) => WaterEntryModel.fromJson(e as Map<String, dynamic>)).toList() : [];
      await _saveToPrefs();
      notifyListeners();
    } catch (e) {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      if (entriesJson != null && savedDate == today) {
        ...
        todayEntries = decoded.map((e) => WaterEntryModel.fromJson(e)).toList();
      } else {
        todayEntries = [];
      }
  ...
  Future<bool> addWater(double liters, {String cardTitle = "", String iconName = "droplet"}) async {
    ...
    try {
      ...
      todayEntries.add(WaterEntryModel(
        cardTitle: cardTitle,
        iconName: iconName,
        liters: liters,
        time: DateTime.now(),
